...
Scroller.prototype = {
	...
	_addEvent: function( ) {
		...
		self._scroller.addEventListener( "touchmove", function( e ) {
			var touches = e.touches,
				hs = touches[0].pageX - self._ex, // horizontal move space
				vs = touches[0].pageY - self._ey; // vertical move space;
			...
			if ( self._dragging ) {
				...
				case "all":
					...
					if ( Math.abs( hs ) < self.options.threshold ) {
						self._sx = self._lastX;
						return;
					}
				}
				//valid degree and over horizontal threshold
				self._sx = self._lastX + hs;
				if ( self._sx > 0 ) {
					// left side
					self._sx = 0;
