/* global expect, document, tau, define, module, test, strictEqual, initFixture, window, ok */
(function () {
	"use strict";
	function runTests(testedObjects, helpers) {
		var engine = testedObjects.engine,
			GridView = testedObjects.GridView,
		...
		test("constructor", function () {
			...
			gridView._configure();
			strictEqual(gridView.options.cols, 4, "Option header is set correct");
			...
			strictEqual(gridView.options.maxCols, 5, "Option footer is set correct");
		});
	...
	if (typeof define === "function") {
		...
		runTests({
			engine: tau.engine,
			GridView: tau.widget.mobile.GridView,
