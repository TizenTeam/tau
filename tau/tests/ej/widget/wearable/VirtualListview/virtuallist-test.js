...
(function() {
	...
	// RUN TEST 1
	test("ej.widget.wearable.VirtualList draw method", 9, function () {
	...
	// RUN TEST 2
	test("ej.widget.wearable.VirtualList scrollToIndex method", 6, function () {
	...
	// RUN TEST 3
	test("ej.widget.wearable.VirtualList fastOn scroll action", 2, function () {
		var scrollview = elList.parentNode,
			li,
			vList = tau.widget.VirtualListview(elList);
	...
	// RUN TEST 4
	asyncTest("ej.widget.wearable.VirtualList tap methods", 3, function () {
	...
	// RUN TEST 5
	test("ej.widget.wearable.VirtualList scrollTo method",  function () {
		var scrollview = elList.parentNode,
			vList = tau.widget.VirtualListview(elList);

		vList.scrollTo(0);
		ej.utils.events.trigger(scrollview, 'scroll');
	...
	// RUN TEST 6
	test("ej.widget.wearable.VirtualList destroy method", 4, function () {
		var children = elList.children,
			nextDiv = elList.nextElementSibling,
			vList = tau.widget.VirtualListview(elList);
	...
	// RUN TEST 7
	test("ej.widget.wearable.VirtualList buffer sizing",  function () {
		var vList,
			config;
