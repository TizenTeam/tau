...
(function (document, window, ns) {
	"use strict";
			 ...
			 * @param {boolean} setBounceBack to set effect of bouncing in over scrolled
	...
	"use strict";
			...
			function _updateList(self, event) {
				var list = self.element,
					...
					scrollChildStyle = self._ui.scrollview.firstElementChild.style,
					fromIndex = 0,
					bounceBack = self.options.bounceBack,
					dataLength = self.options.dataLength,
					map = [],
					freeElements,
					numberOfItems = self._numberOfItems,
					i = 0,
				...
				if (scrollBegin !== undefined) {
					...
					currentIndex = floor(scrollBegin / itemSize);
					if (bounceBack || currentIndex >= 0) {
						if (currentIndex !== floor(self._scrollBeginPrev / itemSize) && currentIndex >= 0) {
							if (scrollBegin < itemSize) {
								fromIndex = 0;
								correction =  0;
						...
						scrollChildStyle.transform = "translate(" + (-scroll.scrollLeft) + "px, " + (-scroll.scrollTop) + "px)";
					}
